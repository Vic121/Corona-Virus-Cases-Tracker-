<?php
if(!class_exists("CoronaVirusCasesTrackerBase")) {
	class CoronaVirusCasesTrackerBase {
    	public $key = "5F9B14CEDE81A706";
    	private $product_id = "9";
    	private $product_base = "corona-virus-cases-tracker";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_corona-virus-cases-tracker_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=corona-virus-cases-tracker_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
